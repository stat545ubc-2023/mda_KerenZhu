...
Here I choose to explore the number of elevators at each apartment building. I believe the outcome can benefit future research on architecture design, facilities for people with mobility difficulties, resource distribution, etc.
...
missing_long<-gather(missing_counts, variable_names, missing_count)
#make a plot
missing_plot<-ggplot(missing_long, aes(x=missing_count,y=reorder(variable_names,missing_count))) +
  geom_col()+
...
```{r}
#create a new tibble and clean the missing value in pets_allowed indicated by Exercise B
property_pets<-apt_buildings%>%
  select(property_type,pets_allowed)%>%
  filter(!is.na(pets_allowed))
#create a contingency table and conduct chi-square test
contingency_table <- table(property_pets$pets_allowed, property_pets$property_type)
...
mosaic_plot <- ggplot(contingency_df, aes(x = factor(PropertyType), y = Percentage, fill = PetsAllowed)) +
  geom_col() +
